...
export default function ShipmentCard({ shipment, selected, onClick, onViewDetails }) {
  const { shipmentId, status, routeLabel, nearestCity, cargo, speedKmh, etaMinutes, progress, priority, cargoValue } = shipment;
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '12px 12px 10px 14px' }}>
        ...
        {/* progress bar */}
        <div style={{ height: '2px', background: 'var(--border)', borderRadius: '1px', marginBottom: '8px' }}>
          <div style={{ width: `${pct}%`, height: '100%', background: cfg.color, boxShadow: `0 0 6px ${cfg.color}`, borderRadius: '1px', transition: 'width 0.6s ease' }} />
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: '14px' }}>
            <div>
              ...
              <div style={{ fontFamily: 'var(--text-mono)', fontSize: '11px', color: status === 'DELAYED' ? 'var(--neon-red)' : 'var(--neon-cyan)' }}>
          ...
          {/* View Details button */}
          <button
            onClick={e => { e.stopPropagation(); onViewDetails && onViewDetails(shipmentId); }}
            style={{
